...
def preprocess_dressipy(dataset_dir,train_for_leaderboard=False):
    ...
    if dataset_dir_str[-1]=="/":
        dataset_dir_str=dataset_dir_str[:-1]
    df_s = pd.read_csv(
        f'{dataset_dir_str}/train_sessions.csv',
        delimiter=',',
        parse_dates=['date'],
        infer_datetime_format=True,
    )
    df_p = pd.read_csv(
        f'{dataset_dir_str}/train_purchases.csv',
        delimiter=',',
        parse_dates=['date'],
        infer_datetime_format=True,
    )
    ...
    if train_for_leaderboard:
        ...
        df = pd.read_csv(
            f'{dataset_dir_str}/test_final_sessions.csv',
            delimiter=',',
            parse_dates=['date'],
            infer_datetime_format=True,
        )
        ...
        d=7
        print(f"Using last {d} days for df_test")
        df_train, df_test = split_by_time(df, pd.Timedelta(days=d))
